...
class MixedAudioCapture {
    ...
    async startRecording(sessionId) {
        try {
            ...
            // Note: macOS requires "Include Microphone" to be enabled in Screen Recording permissions
            const constraints = {
                audio: {
                    mandatory: {
                        chromeMediaSource: 'desktop',
                        chromeMediaSourceId: sources[0].id,
                        // These might help capture both streams
                        echoCancellation: false,
                        noiseSuppression: false,
                        autoGainControl: false
                    }
            ...
            if (!recorderOptions) {
                console.log('⚠️ No specific audio MIME type supported, using default');
                recorderOptions = {}; // Let MediaRecorder choose
            }
            ...
            try {
                ...
                console.log('   - Actual mimeType:', this.recorder.mimeType);
